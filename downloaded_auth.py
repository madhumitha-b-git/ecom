...
def verify_admin_token(credentials: HTTPAuthorizationCredentials = Security(security)) -> str:
    ...
    try:
        ...
        if "." in token and len(token.split(".")) == 3:
            ...
            parts = decoded_str.split(":")
            if len(parts) == 2:
                payload = {"username": parts[0], "role": "user"}

        username = payload.get("email") or payload.get("username") or payload.get("sub", "unknown")
        role = payload.get("role")
        is_admin = (
            role == "admin"
            or username in ("admin", "admin@gmail.com", "madhumithamalu6@gmail.com", "mock_admin")
