...
def create_payment(payment: Payment) -> dict:
    ...
    item = _to_decimal({"payment_id": payment_id, **payment.model_dump(), "status": "SUCCESS", "timestamp": timestamp})
    get_table().put_item(Item=item)
    logger.info("Payment SUCCESS | payment_id=%s order_id=%s", payment_id, payment.order_id)
    return {"payment_id": payment_id, "status": "SUCCESS", "timestamp": timestamp}
